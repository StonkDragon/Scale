...
### static function List::mapIndexed<[T], lambda(T, int): U>(arr: [T], fn: lambda(T, int): U): [U]
Returns a new array containing the results of applying `fn` to each element of `arr` and its index.
The index is the second argument to `fn`.
Provides compatibility with `do 'mapIndexed`.

### static function List::reduce<[T], >(result: T, arr: [T], fn: lambda(T, T): T): T
...
The accumulated value is initialized to `result`.
