...
### function str:Returns the character at index `i` in `self`.
Method is deprecated in favor of `str:[]`.
...
Method is deprecated in favor of `str:view`.
...
### static function str::vformat(fmt: str, args: [any]): str
Formats the string `fmt` with the given arguments.
The format string `fmt` is a string that may contain format specifiers.
Format specifiers are replaced by the values in `args`.
The format specifiers are introduced by a `%` character.
The following format specifiers are supported:
- `"%s"`: string
- `"%i"`: integer
...
### static function str::mapIndexed(:str, mapper: lambda(int8, int): int8): str
Returns a new instance of `str` containing the characters of `self` mapped by `mapper`.
This function makes the `str` type compatible with `do`-notation.

### static function str::reduce(result: int, :str, reducer: lambda(int, int8): int): int
Returns the result of reducing `self` with `reducer` and `result` as the initial value.
This function makes the `str` type compatible with `do`-notation.
